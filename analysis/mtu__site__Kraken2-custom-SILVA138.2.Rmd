...
> `Silva138.2-Kraken2` database: [skip to code only](#r-re-format-silva)
...
Here we take the most recent release of the [Silva](https://www.arb-silva.de/) database (v.138.2 Silva; 2.2M sequences, [varying lengths](https://www.arb-silva.de/documentation/release-1382#c4414)), and format it for use explicitly with the Kraken2 classifier. 

See also all the roundabout talk regarding [the GG2 version](./documents/k2db_from_gg2.html), including a basic [intro to building Kraken2 databases](./documents/k2db_from_gg2.html#intro-to-kraken2-databases).
...
__of note:__ 
  ...
  4.    We use `SILVA_138.2_SSU`**`Ref`**`tax_silva.fasta.gz`, and not the `NR99` version (tiny differences are the entire point of using ASVs). However, mighty [`benjjneb` and co.](https://benjjneb.github.io/dada2) use NR99 for their NB-RDP method. Should we follow suit? Probably not - [DADA2 uses a](https://benjjneb.github.io/dada2/assign.html) [twostep](https://miakoden.bandcamp.com/album/keep-it-steppin) [solution](https://benjjneb.github.io/dada2/assign.html), whereas we are aiming for perfect-firstime-e'rytime.
...
The reference `FASTA` sequences, complete with all-important header:
  ...
  - the sequence accession, complete with version numbering. Note that the `primaryAccession` in the `taxmap` file (#2 above) does __not__ contain version numbering (e.g., `AY846372`), while the accession in the `seqs.fna` header does (e.g. `AY846372.1.1779`). We will have to either remove the versioning, or edit the header to start with just the accession (i.e., "`>AY846372|...`, and *not* "`>AY846372`**`.1.1779`**`|...` )
...
# Build S138.2 database to species-level - tasks
...
From `Silva`, we need to create the following using the `taxid`s we copy from Silva/create for species level: 
  ...
  -   `seqs.fna`:   `FASTA`-format DNA sequences, with the header __reformatted__ as: "`>---#1---|kraken:taxid|---#2---`", where `---#1---` is the accession the sequence came with, and `---#2---` is the `taxid` value (either the one copied from `Silva`, or the one we create for species-level assignment). Any additional fields must be separated with a pipe (`|`), and __spaces are not allowed in the header__.

---        

# Build S138.2 database to species-level - `code`        
...
> __NB:__ Building a database from scratch skips the first step in the standard database building (e.g. `kraken2-build --download-library bacteria --db $k2db`)
...
Now try building the database:    CHECK PRELIMMAP again! compare with GG2! 
...
# Download S138.2-Kraken2 database
...
[Still processing... ~~Download here (Zenodo.org)~~](https://zenodo.org/records/17738675?preview=1&token=eyJhbGciOiJIUzUxMiJ9.eyJpZCI6ImM4NmViOGIzLTQzNzAtNGRjMi1iNTlmLWM0MmYzNDU1YWNjMCIsImRhdGEiOnt9LCJyYW5kb20iOiI2OGM2NmU1OWEwYjg0NmUxMGMzOTg2ZWViZjRhM2M0OCJ9.rSOPiIiK8rl8Jn1t5qp_UhL0zUhO9JJqnGZ5gL1oJvNLY34Mq937KTekQtTPz7Kw7O22Z8EkgLIoOYCLXDyTVA)
...
Things to be aware of if using this data for your work:
  ...
  -   Cake is a made up drug[.](https://youtu.be/k0tMvxV-GC4?si=yQIageBDkjnXyoTA&t=223) We made completely fake `taxid`s, so __none of these will match NCBI__ taxonomy (but none of these 16S databases match that anyway).
