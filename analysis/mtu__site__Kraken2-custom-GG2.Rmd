...
> Just the code?  - [see end of page](#r)

> Just download the database?  - [see end of page](#download-greengenes2-331k-for-kraken2)
...
Greengenes was a popular and comprehensive 16S database representing 84K (non-unique) sequences. Greengenes has been recently updated to a staggering 2M full-length (+21M V4 sequences) ([GG2, 2024](https://greengenes2.ucsd.edu)).

GG2 is not natively available for use with `Kraken2`, everyone's favourite _k_-mer based taxonomic classifier.
...
The `taxonomy.id` file should look like below: the long accessions are NCBI, and the short numbers are from the 23M V4 sequences added to the tree. Note the poor confidence scores: 0.3. It's not clear how "`confidence`" is estimated but 0.3 don't look great. Note also that `Taxon` is a _single column_, with the different `d__; c__; o__;` ranks joined together by whitespace. 
...
###### Download GreenGenes2 331K for Kraken2
  ...
  -   `hash.k2d` - 38.4MB ([no link yet!]())
  -   `opts.k2d` - 64B ([no link yet!]())
  -   `taxo.k2d` - 1.5MB ([no link yet!]())
  
Or even so far as want to go to

  -   `full_db_tar.gz` - includes nodes and map, ...MB ([no link yet!]())
